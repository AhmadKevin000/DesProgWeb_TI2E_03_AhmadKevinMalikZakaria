...
if (!empty($_GET['anggota'])) {
    ...
    $hased_password = password_hash($combined_password, PASSWORD_BCRYPT);

    $query = "INSERT INTO user (username, password, salt, level) VALUES ('$username', '$hased_password', '$salt', '$level')";
    ...
    if (mysqli_query($koneksi, $query)) {
        ...
        if (mysqli_query($koneksi, $query2)) {
            ...
            pesan('warning', "User berhasil dibuat tapi data anggota gagal: " . mysqli_error($koneksi));
        }

    } else {
        pesan('danger', "Gagal menambahkan user: " . mysqli_error($koneksi));
    }
